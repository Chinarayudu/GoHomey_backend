...
async function resolveChefFromStreamToken(req: Request) {
  ...
  const payload = jwt.verify(token, process.env.JWT_SECRET || 'super-secret-key') as any;
...
fuelRouter.post(
  ...
  async (req, res, next) => {
    try {
      const result = await fuelService.createSubscription((req.user as any).id, req.body);
      res.status(201).json(result);
    } catch (error) {
      next(error);
    }
...
fuelRouter.get('/chef/subscriptions', jwtAuth, checkRoles(Role.CHEF), async (req, res, next) => {
  try {
...
fuelRouter.get('/chef/fulfillments', jwtAuth, checkRoles(Role.CHEF), async (req, res, next) => {
  try {
    const chef = await resolveChefFromRequest(req);
    if (!chef) {
      return res.status(403).json({ status: 'error', message: 'Chef profile not found' });
    }
    const result = await fuelService.listChefFulfillments(chef.id, req.query.date as string | undefined);
    res.json(result);
  } catch (error) {
    next(error);
  }
...
fuelRouter.get('/now/dispatch/:id', jwtAuth, async (req, res, next) => {
  try {
    ...
    if (!session) {
      return res.status(404).json({ status: 'error', message: 'Fuel NOW dispatch session not found' });
    }
    res.json(session);
  } catch (error) {
    next(error);
...
fuelRouter.post('/now/dispatch/:id/respond', jwtAuth, checkRoles(Role.CHEF), async (req, res, next) => {
  try {
    const chef = await resolveChefFromRequest(req);
    if (!chef) {
      return res.status(403).json({ status: 'error', message: 'Chef profile not found' });
...
fuelRouter.post(
  ...
  async (req: Request, res: Response, next) => {
    try {
      ...
      if (!req.file) {
        return res.status(400).json({ status: 'error', message: 'batch_proof photo is required' });
      }

      const grams = Number(req.body.weight_verification_grams);
      const uploadedProof = await cloudinaryService.uploadFile(req.file, 'homey/fuel/weigh-ins');
...
fuelRouter.post('/fulfillments/generate', jwtAuth, checkRoles(Role.ADMIN), async (req, res, next) => {
  try {
    const daysAhead = req.body?.daysAhead ? Number(req.body.daysAhead) : undefined;
...
fuelRouter.post('/reminders/prep/run', jwtAuth, checkRoles(Role.ADMIN), async (req, res, next) => {
  try {
    const hoursBefore = req.body?.hoursBefore ? Number(req.body.hoursBefore) : 3;
